...
const handleClickMentorOne = (e)=>{
  ...
  if(selectedMentor === mentorOne){
    axios.patch(`/api/users/${userId}/addMentor`, mentorOne)
...
const handleClickMentorTwo = (e)=>{
  ...
  if(selectedMentor === mentorTwo){
    axios.patch(`/api/users/${userId}/addMentor`, mentorTwo)
  ...
  useEffect(()=>{  
        axios.get(`/api/users/${userId}/matchMentors?page=0&limit=10`)
        .then((mentors)=>setMentors(mentors.data))
